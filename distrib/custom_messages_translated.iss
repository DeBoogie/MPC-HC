...
ar.WinVersionTooLowError=[name] يتطلب Windows 10 او أحدث لتشغيله
...
be.WinVersionTooLowError=[name] requires Windows 10 or newer to run.
...
;bg.WinVersionTooLowError=[name] изисква Windows 10 или по-нова версия за да се изпълни.
...
bn.WinVersionTooLowError=[name] requires Windows 10 or newer to run.
...
bs_BA.WinVersionTooLowError=[name] za rad zahtijeva Windows 10 ili noviji.
...
ca.WinVersionTooLowError=[name] requereix Windows 10 o superior perquè funcioni.
...
cs.WinVersionTooLowError=[name] vyžaduje ke svému běhu Windows 10 nebo novější.
...
da.WinVersionTooLowError=[name] kræver Windows 10 eller nyere for at køre.
...
de.WinVersionTooLowError=Systemvoraussetzung ist Windows 10 oder neuer.
...
el.WinVersionTooLowError=Το [name] απαιτεί Windows 10 ή νεότερα για να λειτουργήσει.
#endif
...
en_GB.WinVersionTooLowError=[name] requires Windows 10 or newer to run.
...
es.WinVersionTooLowError=[nombre] Para ejecutar se requiere Windows 10 o superior
...
eu.WinVersionTooLowError=[name] Windows 10 edo berriagoa behar du lan egiteko.
...
fi.WinVersionTooLowError=[name] requires Windows 10 or newer to run.
...
hr.WinVersionTooLowError=[name] zahtjeva Windows 10 ili novije za rad.
...
hu.WinVersionTooLowError=[name] használatához Windows 10 vagy újabb operácios rendszer használata szükséges.

; Armenian (Armenia)
hy.WelcomeLabel2=[name]-ը կտեղադրվի ձեր համակարգչում։%n%nԽորհուրդ ենք տալիս փակել մյուս բոլոր ծրագրերը։
hy.WinVersionTooLowError=[name]-ը պահանաջում է Windows 10 կամ ավելի նորը:
...
id.WinVersionTooLowError=[name] membutuhkan Windows 10 atau lebih baru untuk dapat berjalan.
...
it.WinVersionTooLowError=[name] richiede per l'esecuzione Windows 10 o versioni successive.

; Japanese
ja.WelcomeLabel2=これにより、コンピュータに [name] がインストールされます。%n%n続行する前に他のすべてのアプリケーションを閉じることを推奨します。
ja.WinVersionTooLowError=[name] を実行するには Windows 10 以降が必要です。

; Korean
ko.WelcomeLabel2=이것은 [name] 를(을) 당신의 컴퓨터에 설치합니다.%n%n설치를 계속하기 전에 다른 모든 프로그램을 종료하는 것을 권장합니다.
ko.WinVersionTooLowError=[name] requires Windows 10 or newer to run.
...
lt.WinVersionTooLowError=[name] requires Windows 10 or newer to run.
...
ms_MY.WinVersionTooLowError=[name] memerlukan Windows 10 atau yang lebih baharu untuk dijalankan.
...
nl.WinVersionTooLowError=[name] vereist Windows 10 of hoger.
...
pl.WinVersionTooLowError=[name] wymaga systemu Windows 10 lub nowszego do uruchomienia.
...
pt_BR.WinVersionTooLowError=O [name] requer Windows 10 ou mais novo pra rodar.
...
pt_PT.WinVersionTooLowError=O [name] requer o Windows 10 ou posterior para funcionar.
...
ro.WinVersionTooLowError=[name] necesită Windows 10 sau mai nou pentru a rula.
...
ru.WinVersionTooLowError=Для запуска [name] требуется Windows 10 или выше.
...
sk.WinVersionTooLowError=[name] vyžaduje na svoje fungovanie systém Windows 10 alebo novší.

; Slovenian
sl.WelcomeLabel2=[name] bo nameščen na tem računalniku.%n%nPriporočamo, da zaprete vse ostale programa pred nadaljevanjem.
sl.WinVersionTooLowError=[name] requires Windows 10 or newer to run.
...
sr.WinVersionTooLowError=[name] requires Windows 10 or newer to run.
#endif
...
sv.WinVersionTooLowError=[namn] kräver Windows 10 eller nyare för att köras.

; Thai (Thailand)
th_TH.WelcomeLabel2=สิ่งนี้จะติดตั้ง [name] บนคอมพิวเตอร์ของคุณ%n%nขอแนะนำให้คุณปิดโปรแกรมอื่นๆ ก่อนทำขั้นตอนต่อไป
th_TH.WinVersionTooLowError=[name] requires Windows 10 or newer to run.
...
tr.WinVersionTooLowError=[name] programını çalıştırmak için Windows 10 veya daha yeni bir sürüm gerekiyor.
...
tt.WinVersionTooLowError=[name] requires Windows 10 or newer to run.
...
uk.WinVersionTooLowError=[name] requires Windows 10 or newer to run.
...
vi.WinVersionTooLowError=[name] yêu cầu Windows 10 hoặc mới hơn để chạy.

; Chinese (P.R.C.)
zh_CN.WelcomeLabel2=这将安装 [name] 到您的计算机中。%n%n建议您在继续之前关闭所有其它的应用程序。
zh_CN.WinVersionTooLowError=[name] 需要运行在Windows 10或更新版本的Windows系统上

; Chinese (Taiwan)
zh_TW.WelcomeLabel2=這將會安裝 [name] 在您的電腦中。%n%n建議您關閉所有其它應用程式後再繼續。
zh_TW.WinVersionTooLowError=[name] 需要 Windows 10 或更新版才能執行。
