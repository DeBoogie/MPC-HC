...
FOR %%G IN (%ARG%) DO (
  ...
  IF /I "%%G" == "VS2022"   SET "COMPILER=VS2022"     & SET /A ARGCOMP+=1
...
IF %ARGCOMP% GTR 1 (GOTO UnsupportedSwitch) ELSE IF %ARGCOMP% == 0 (SET "COMPILER=VS2022")
...
ECHO.
ECHO Usage:
ECHO %~nx0 [Clean^|Build^|Rebuild] [x86^|x64^|Both] [Debug^|Release] [VS2022]
ECHO.
...
ECHO "%~nx0 Build Both Release VS2022"
ECHO.
POPD
ENDLOCAL
EXIT /B
