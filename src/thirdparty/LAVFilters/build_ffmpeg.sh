...
configure() {
  ...
  if [ "${arch}" == "x86_64" ]; then
    ...
    if [ "${COMPILER}" == "MSVC" ]; then
      ...
      EXTRA_CFLAGS="-D_WIN32_WINNT=0x0A00 -DWINVER=0x0A00 -Zo -GS"
      ...
      EXTRA_CFLAGS="-fno-tree-vectorize -D_WIN32_WINNT=0x0A00 -DWINVER=0x0A00 -gdwarf-5 -fno-omit-frame-pointer"
